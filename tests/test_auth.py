...
class AuthTestCase(unittest.TestCase):
    ...
    def test_registration(self):
        ...
        with self.app:
            response = self.app.post('/v1/auth/register', data=json.dumps(self.user_data),
    ...
    def test_register_with_already_registered_user(self):
        ...
        res = self.app.post('/v1/auth/register', data=json.dumps(self.user_data),
                            content_type='application/json')
    ...
    def test_user_login(self):
        ...
        res = self.app.post('/v1/auth/register', data=json.dumps(self.user_data),
                            content_type='application/json')
        self.assertEqual(res.status_code, 201)
        login_res = self.app.post('/v1/auth/login', data=json.dumps(self.user_data),
                                  content_type='application/json')
    ...
    def test_non_registered_user_login(self):
        ...
        res = self.app.post('/v1/auth/login', data=json.dumps(self.user_data),
                            content_type='application/json')
    ...
    def test_user_can_reset_password(self):
        ...
        res = self.app.post('/v1/auth/register', data=json.dumps(self.user_data),
                            content_type='application/json')
        self.assertEqual(res.status_code, 201)
        rese = self.app.post('/v1/auth/reset-password', data=json.dumps({"email": "test@gmail.com", "password": "new"}),
                             content_type='application/json')
    ...
    def test_non_user_cannot_reset_password(self):
        ...
        res = self.app.post('/v1/auth/register', data=json.dumps(self.user_data),
                            content_type='application/json')
        self.assertEqual(res.status_code, 201)
        rese = self.app.post('/v1/auth/reset-password', data=json.dumps({"email": "none@gmail.com", "password": "new"}),
                             content_type='application/json')
    ...
    def test_user_logout(self):
        ...
        login_res = self.app.post(
            '/v1/auth/login', data=json.dumps(self.user_data), content_type='application/json')
        access_token = json.loads(login_res.data.decode())['access_token']
        logout_res = self.app.post(
            '/v1/auth/logout', headers={'Content-Type': 'application/json', 'access-token': access_token})
